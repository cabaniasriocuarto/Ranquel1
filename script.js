...
const translations = {
  es: {
    ...
    'booking.step2.desc': 'Si ya reservaste, usá el link que te llegó por email. Si estás probando, podés entrar a una sala demo.',
    'booking.room.label': 'Sala / Código',
    'booking.room.placeholder': 'ej: ranquel-consulta',
    'booking.room.btn': 'Entrar',
    'booking.room.help': 'Tip: si no ponés nada, entra a <strong>ranquel-consulta</strong>.',
    ...
    'booking.pro.title': 'Automatización Pro (opcional)',
    'booking.pro.desc': 'Si querés que cada turno cree una sala única en Daily y se agregue al evento automáticamente, dejé un instructivo y script en <code>docs/daily-google-calendar.md</code>.',
'cta.title': '¿Consultas? Para Mayor Información Usá el ChatBot',
  ...
  en: {
    ...
    'booking.step2.desc': 'If you already booked, use the link you received by email. If you are testing, you can join a demo room.',
    'booking.room.label': 'Room / Code',
    'booking.room.placeholder': 'e.g. ranquel-consulta',
    'booking.room.btn': 'Join',
    'booking.room.help': 'Tip: if empty, we will join <strong>ranquel-consulta</strong>.',
    ...
    'booking.pro.title': 'Pro automation (optional)',
    'booking.pro.desc': 'If you want each booking to create a unique Daily room and attach it to the event automatically, see <code>docs/daily-google-calendar.md</code>.',
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function buildDailyUrl(value) {
    ...
    const fallbackRoom = 'ranquel-consulta';

    if (!raw) {
      return `https://ranquel.daily.co/${encodeURIComponent(fallbackRoom)}`;
    }

    // Si el usuario pega una URL completa
    if (/^https?:\/\//i.test(raw)) return raw;

    // Si pega algo como ranquel.daily.co/room
    if (raw.includes('daily.co')) {
      const cleaned = raw.replace(/^\/\//, '');
      return cleaned.startsWith('http') ? cleaned : `https://${cleaned}`;
    }

    // Si pega solo el nombre de sala
    return `https://ranquel.daily.co/${encodeURIComponent(raw)}`;
  }

  function renderDaily(roomOrUrl) {
    if (!dailyContainer) return;
    const url = buildDailyUrl(roomOrUrl);
  ...
  // Deep link: /?view=reservas&room=xxx
  try {
    ...
    if (view === 'reservas') {
      navegarA('reservas');
      if (room) {
        if (dailyRoomInput) dailyRoomInput.value = room;
        renderDaily(room);
      }
    }
  } catch (_) {}
